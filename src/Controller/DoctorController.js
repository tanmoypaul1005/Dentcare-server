...
module.exports.AddDoctor=(req,res)=>{
   const{ name,email,contactNumber,position,degree1,degree2,degree3,degree4,description,experience1,experience2,experience3}=req.body; 
   Doctor.findOne({email:email})
   .exec((error,data)=>{
    if(data){return res.status(201).json({msg:'Doctor All Rady Register'})}

    const _doctor=new Doctor({name,username:slugify(name).toLowerCase(),email,contactNumber,position,degree1,degree2,degree3,degree4,description,experience1,experience2,experience3});
